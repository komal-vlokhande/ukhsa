...
const mapDispatchToProps = (dispatch: any) => ({
  getAuthenticationDetails: (reqData) =>
    dispatch(getAuthenticationDetails(reqData)),
});
...
class Authentication extends React.Component<any, any> {
  ...
  handleSubmit = async (e) => {
    ...
    if (errorObj.children != "") {
      ...
      const DOB = moment(`${day}-${month}-${year}`, "DD-MM-YYYY").format(
        "DDMMYYYY"
      );
      await this.props.getAuthenticationDetails({
        dob: DOB,
        urlToken: this.state.access_token[2],
      });
      let responseData = ValidateBackendResponse(this.props.authDetails);
      if (responseData.length > 0) {
        this.setState({
          errors: responseData[0].error.map((value) => {
            return { children: value };
          }),
        });
        this.setState({
          errorMessage: { children: responseData[1].errorMessage },
        });
      } else {
